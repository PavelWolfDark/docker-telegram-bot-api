...
ARG TELEGRAM_BOT_API_VERSION=6.4

FROM ubuntu:${UBUNTU_VERSION} AS ubuntu

FROM alpine:${ALPINE_VERSION} AS alpine

FROM ubuntu AS ubuntu-telegram-bot-api-6.4-build
ENV TELEGRAM_BOT_API_VERSION=6.4
ENV TDLIB_VERSION=1.8.10
RUN \
  set -eux \
  && apt-get update\
...
RUN \
  ...
  && cd tdlib \
  && git reset --hard '93c42f6d7c1209937431469f80427d48907f1b8d'
RUN \
  set -eux \
  && git clone https://github.com/tdlib/telegram-bot-api.git telegram-bot-api \
  && cd telegram-bot-api \
  && git reset --hard 'c3b3c3033fe867810e08b80f077f128bc932b7fc'
WORKDIR /dist
RUN \
  set -eux \
...
FROM scratch AS ubuntu-telegram-bot-api-6.4-release
COPY --from=ubuntu-telegram-bot-api-6.4-build /dist /

FROM ubuntu AS ubuntu-telegram-bot-api-6.4-deploy
ARG UID=1000
ARG GID=1000
RUN \
  set -eux \
  ...
  && apt-get install --no-install-recommends -y \
    ...
    gosu \
  && rm -rf /var/lib/apt/lists/*
COPY --from=ubuntu-telegram-bot-api-6.4-release / /usr/
RUN \
  set -eux \
...
FROM alpine AS alpine-telegram-bot-api-6.4-build
ENV TELEGRAM_BOT_API_VERSION=6.4
ENV TDLIB_VERSION=1.8.10
RUN \
  set -eux \
...
FROM scratch AS alpine-telegram-bot-api-6.4-release
COPY --from=alpine-telegram-bot-api-6.4-build /dist /

FROM alpine AS alpine-telegram-bot-api-6.4-deploy
ARG UID=1000
ARG GID=1000
RUN \
  set -eux \
  && apk add --no-cache \
    ...
    openssl \
    su-exec
COPY --from=alpine-telegram-bot-api-6.4-release / /usr/
RUN \
  set -eux \
